...
CREATE TABLE `user` (
  ...
  `mfa_enabled`         BOOLEAN       DEFAULT FALSE COMMENT '是否启用了 MFA',
...
-- CREATE UNIQUE INDEX `uk_username_deleted` ON `user` (`username`, `deleted_at`);
-- CREATE UNIQUE INDEX `uk_email_deleted` ON `user` (`email`, `deleted_at`);
-- CREATE UNIQUE INDEX `uk_phone_deleted` ON `user` (`phone_country_code`, `phone_number`, `deleted_at`);
...
-- CREATE UNIQUE INDEX `uk_email_active` ON `user` (`email`) WHERE `deleted_at` IS NULL;
-- CREATE UNIQUE INDEX `uk_phone_active` ON `user` (`phone_country_code`, `phone_number`) WHERE `deleted_at` IS NULL;
...
-- CREATE UNIQUE INDEX `uk_username_active` ON `user` (`username`, (IFNULL(`deleted_at`, 1)));
-- CREATE UNIQUE INDEX `uk_email_active` ON `user` (`email`, (IFNULL(`deleted_at`, 1)));
-- CREATE UNIQUE INDEX `uk_phone_active` ON `user` (`phone_country_code`, `phone_number`, (IFNULL(`deleted_at`, 1)));

-- ✅ 正确写法：“索引表达式”。
-- 类比理解：“只监控活着的用户名，死了的随便重名”。符合软删除业务中最常见且合理的唯一性需求。
CREATE UNIQUE INDEX uk_username_active ON user (
  (CASE WHEN deleted_at IS NULL THEN username ELSE NULL END)
);

CREATE UNIQUE INDEX uk_email_active ON user (
  (CASE WHEN deleted_at IS NULL THEN email ELSE NULL END)
);

CREATE UNIQUE INDEX uk_phone_active ON user (
